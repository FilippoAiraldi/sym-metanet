...
"""Holds METANET configuration (see class type below)"""
_CONFIG = None
...
def init(config: Config) -> None:
    ...
    for attr in ('lanes', 'L', 'C0'):
        ...
        if isinstance(v, np.ndarray):
            setattr(config, attr, tm_util.force_2d(v))
    global _CONFIG
    _CONFIG = config
    return _CONFIG
...
class links(tm_util._NonInstantiable):
    ...
    def get_Veq(rho):
        ...
        '''
        a, v_free, rho_crit = _CONFIG.a, _CONFIG.v_free, _CONFIG.rho_crit
    ...
    def step_density(rho, q, q_up, r, s):
        ...
        '''
        return rho + (_CONFIG.T * (q_up - q + r - s) /
                      (_CONFIG.lanes * _CONFIG.L))
    ...
    def step_speed(v, v_up, Veq, rho, rho_down, r):
        ...
        '''
        lanes, L, T, tau, kappa = (
            _CONFIG.lanes, _CONFIG.L, _CONFIG.T, _CONFIG.tau, _CONFIG.kappa)
        lanes_down = np.array([*lanes[1:], lanes[-1]])
        # equilibrium speed error
        t1 = (T / tau) * (Veq - v)
        # on-ramp merging phenomenum
        t2 = (_CONFIG.delta * T) * (r * v) / (L * lanes * (rho + kappa))
        # speed difference error
        t3 = T * (v / L) * (v_up - v)
        # density difference error
        t4 = (_CONFIG.eta * T / tau) * (rho_down - rho) / (L * (rho + kappa))
        # lane drop phenomenum
        t5 = (_CONFIG.phi * T) * (((lanes - lanes_down)
                                   * rho * cs.power(v, 2))
                                  / (L * lanes * _CONFIG.rho_crit))
        return v + t1 - t2 + t3 - t4 - t5

    @staticmethod
    def get_downstream_density(rho):
        return cs.fmin(rho, _CONFIG.rho_crit)
...
class ramps(tm_util._NonInstantiable):
    ...
    def get_flow(u, w, d, rho_first):
        ...
        r_hat = _CONFIG.C0 * cs.fmin(1, (rho_max - rho_first) /
                                     (rho_max - _CONFIG.rho_crit))
        return u * cs.fmin(d + w / _CONFIG.T, r_hat)

    @staticmethod
    @tm_util._nonnegative
    @tm_util._check_shapes_col
    def step_queue(w, d, r):
        return w + _CONFIG.T * (d - r)
...
class util(tm_util._NonInstantiable):
    ...
    def steady_state(flow_in):
        ...
        incoming flow.
        '''
        I, lanes, v_free = _CONFIG.I, _CONFIG.lanes, _CONFIG.v_free
        ...
        G_rho = rho - links.step_density(rho, q, q_up, np.zeros((I, 1)),
                                           np.zeros((I, 1)))

        # next speed should return same speed, i.e., v - v_next = 0
        v_up = cs.vertcat(links.get_upstream_speed(v[0]), v[:-1])
        Veq = links.get_Veq(rho)
        rho_down = cs.vertcat(rho[1:],
                              links.get_downstream_density(rho[-1]))
        G_v = v - links.step_speed(v, v_up, Veq, rho, rho_down,
                                     np.zeros((I, 1)))
