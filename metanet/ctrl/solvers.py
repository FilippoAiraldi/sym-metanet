...
class NlpSolMPC(SolverBase):
    ...
    def _process_solver_inputs(self, vars_init: Dict[str, float],
        ...
        x0.extend((self.perturb(vars_init[slack], lb=0)
                   if perturb else
                   vars_init[slack]) for slack in self.slacks)
    ...
    def _process_solver_output(self,
        ...
        for link, _ in net.links_with_vms:
            ...
            i += link.nb_vms * Nc
        for slackname, slack in self.slacks.items():
            Ns = slack.shape[0] * slack.shape[1]
            out[slackname] = x_opt[i:i + Ns].reshape(slack.shape)
            i += Ns
        assert i == x_opt.shape[0]
        return out, info
